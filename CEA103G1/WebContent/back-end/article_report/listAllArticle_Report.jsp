...
<h2>被檢舉文章總表</h2>
  ...
  <c:forEach var="article_reportVO" items="${list}">
    <tr>
      ...
      <c:forEach var="articleVO" items="${articleDAO.all_Front}">
      ...
      <c:if test="${article_reportVO.proc_stat==1}"><td data-label="PROC_STAT">已通過</td></c:if>
      <c:if test="${article_reportVO.proc_stat==2}"><td data-label="PROC_STAT">未通過</td></c:if>
