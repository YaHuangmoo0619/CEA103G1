...
public class Product_comment_reportDAO implements Product_comment_reportDAO_interface {
	...
	private static final String INSERT_STMT = 
		"INSERT INTO campion.product_comment_report (mbr_no,prod_no,cmnt_cont,cmnt_time,cmnt_stat) VALUES (?, ?, ?, ?, ?)";
	private static final String GET_ALL_STMT = 
		"SELECT prod_cmnt_rpt_no,mbr_no,prod_no,cmnt_cont,cmnt_time,cmnt_stat FROM campion.product_comment_report order by prod_cmnt_rpt_no";
	private static final String GET_ONE_STMT = 
		"SELECT prod_cmnt_rpt_no,mbr_no,prod_no,cmnt_cont,cmnt_time,cmnt_stat FROM campion.product_comment_report where prod_cmnt_rpt_no = ?";
	private static final String DELETE = 
		"DELETE FROM campion.product_comment_report where prod_cmnt_rpt_no = ?";
	private static final String UPDATE = 
		"UPDATE campion.product_comment_report set mbr_no=?, prod_no=?, cmnt_cont=?, cmnt_time=?, cmnt_stat=? where prod_cmnt_rpt_no=?";
	...
	public void insert(Product_comment_reportVO product_comment_reportVO) {
		...
		try {
			...
			pstmt.setInt(2, product_comment_reportVO.getProd_no());
			pstmt.setString(3, product_comment_reportVO.getCmnt_cont());
			pstmt.setTimestamp(4, product_comment_reportVO.getCmnt_time());
			pstmt.setInt(5, product_comment_reportVO.getCmnt_stat());
	...
	public Product_comment_reportVO findByPrimaryKey(Integer prod_cmnt_rpt_no) {
		...
		try {
			...
			while (rs.next()) {
				...
				product_comment_reportVO.setProd_no(rs.getInt("prod_no"));
				product_comment_reportVO.setCmnt_cont(rs.getString("cmnt_cont"));
				product_comment_reportVO.setCmnt_time(rs.getTimestamp("cmnt_time"));
				product_comment_reportVO.setCmnt_stat(rs.getInt("cmnt_stat"));
			}
